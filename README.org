...
| =org-mindmap-edit-node=      | =RET=              | edit node at point (if you just need to fix a typo you may do it     |
|                            |                  | without this command, but when the node length changes significantly |
|                            |                  | the map will likely fail to recover its structure)                   |
| =org-mindmap-insert-child=   | =M-S-RET=, =C-c C-n= | add a child node to the current one                                  |
