...
The package tries to eleminate those gaps, so if you see gaps between vertical connectors on adjacent lines, there could be those culprits:
...
You'll need a font which sets width for those characters to be exactly double of the regular characters, otherwise your map will be visually crumpled, vertical connectors will not align. I've tested =Noto Sans Mono CJK SC= and it seems to work fine.

** Right-to-Left
Left-to-right languages are not supported yet.
