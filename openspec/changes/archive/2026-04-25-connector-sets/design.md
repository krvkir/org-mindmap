...
**Goals:**
- Enable visual customization through connector packs via `org-mindmap-parser-connectors`.
- Maintain backward compatibility for mindmaps using older symbols via `org-mindmap-parser-root-delimiters`.
...
Connectors will be stored in `org-mindmap-parser-connectors` as a list of lists. Each inner list (pack) must contain exactly 11 characters.
...
Rendering will use a fixed vector of symbols derived from the first element of `org-mindmap-parser-connectors`.
