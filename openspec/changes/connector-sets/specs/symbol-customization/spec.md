...
The system SHALL allow users to configure multiple "connector packs" via a list. Each pack must contain the 11 standard box-drawing characters in a fixed order (Horizontal, Vertical, T-Down, T-Up, T-Right, T-Left, Cross, Corner-TL, Corner-TR, Corner-BL, Corner-BR).

#### Scenario: Configuring a new connector pack
- **WHEN** the user adds a new list of 11 characters to `org-mindmap-connector-sets`
- **THEN** those characters become valid for parsing mindmaps

### Requirement: User-defined delimiter sets
The system SHALL allow users to configure multiple root delimiter pairs via a list of cons cells.

#### Scenario: Configuring a new delimiter pair
- **WHEN** the user adds `("«" . "»")` to `org-mindmap-delimiter-sets`
